...
//mail
  ...
  let transporter = nodemailer.createTransport({
      host: 'travel@gmail.com',
      port: 587,
      secure: false,
      requireTLS: true,
      auth: {
          user: 'travelpakistan@gmail.com',
          pass: 'travelpakistan'
      }, tls: {
  ...
  let mailOptions = {
      from: 'travelpakistan@gmail.com',
      to: email,
      subject: 'Your new Travel Destinations',
      text: 'Check our latest Trips at Travel Pakistan',
      html:output
  };
...
async function auth(req,res,next){
    ...
    try{
        let user = jwt.verify(token,"travelpakistan");
...
app.post("/forgetpassword",(req,res)=>{
    User.findOne({email:req.body.email},(err,found)=>{
        ...
        if(!err){
            if(found){
              let token = jwt.sign({_id:found._id},"travelpakistan",{expiresIn:'10m'})
...
app.post("/register",(req,res)=>{
    const output = `
      <h3>Congratulation on Signing Up to Travel Pakistan</h3>
      <h3>Hello ${req.body.name}</h3>
      <p>Enjoy Our daily Trips at Travel Pakistan</p>
    `;
    try{
      User.findOne({email:req.body.email},(err,found)=>{
          if(!err){
               ...
               if(found){
                ...
                bcrypt.hash(req.body.password,saltRounds,(err,hash)=>{
                    ...
                    let token = jwt.sign({_id:user._id,name:user.name,role:user.role},"travelpakistan")
                    let dataToReturn = {
                        name:user.name,
                        email:user.email,
                        phone:user.phone,
                ...
                bcrypt.compare(req.body.password,found.password,(err,result)=>{
                  if(!err){
                      if(!result){
                        ...
                        let token = jwt.sign({_id:found._id,name:found.name,role:found.role},"travelpakistan")
