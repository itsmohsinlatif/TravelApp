...
<form className="w-full max-w-lg"style={{margin:"20px auto",border:"5px #3282b8",borderStyle:"inset",padding:"15px",fontSize:"16px"}}>
  <div className="flex flex-wrap -mx-3 mb-6">
    ...
    <div className="w-full md:w-1/2 px-3">
      <label style={{fontSize:"14px"}} class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">
       Image
      </label>
    ...
    <div className="w-full md:w-1/2 px-3">
      ...
      <input  className="card1 appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-last-name" name="days" type="number"autoComplete="off" value={days}  onChange={(e) => {setDays(e.target.value); }} required />
    </div>
  ...
  <Button   variant="contained"color="primary" className="full btn card1" onClick={(e)=>{
   postService.addPost({heading,place,city,image,address,description,days,price,setDate,arrivalDate,guideName,guideNumber})
   .then((data)=>{
     console.log(data)
...
<p>
 ...
 <Button   variant="contained"color="primary" className="full btn" onClick={(e)=>{
   postService.addPost({heading,place,city,image,address,description,days,setDate,arrivalDate,guideName,guideNumber})
   .then((data)=>{
     console.log(data)
     history.push("/home")
   }).catch((err)=>{
